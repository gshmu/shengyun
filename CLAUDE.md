...
| 行 | 列1 | 列2 | 列3 | 列4 | 列5 | 列6 | 列7 | 列8 | 列9 | 列10 |
|---|-----|-----|-----|-----|-----|-----|-----|-----|-----|------|
| **行1** | **er** | **a** | **an** | **ang** | **o** | **e** | **en** | **ao** | 空 | 空 |
| **行2** | **i** | **ai** | **ian** | **iang** | 空 | **ei** | **ie** | **iao** | **in** | **ia** |
| **行3** | **u** | **ua** | **uai** | **uang** | **ou** | **ui** | **iu** | **iong** | 空 | **⇧** |
| **行4** | **ü** | **ün** | **uan** | **eng** | **uo** | **üe** | **ing** | **ong** | **␣** | **⌫** |

**按键说明**：
- 行1列1：**er** - 零声母专用，可点击输入"er"（儿）
- 行1列2-8：基础韵母层（a, an, ang, o, e, en, ao）
- 行3列10：**⇧** - 切换回声母层（Keyboard_shengyun_initials）
- 行4列2：**ün** - 互补分布（通用层显示"ün"，j/q/x/y层显示"ün"，其他层显示"un"）
- 行4列3：**uan** - 互补分布（通用层显示"uan"，j/q/x/y层显示"üan"，其他层显示"uan"）
- 行4列9：**␣** - 空格键
- 行4列10：**⌫** - 删除键
...
|---|-----|-----|-----|-----|---------|
| 1 | er | i | u | ü | **基础元音完美垂直** ✓ |
| 2 | **a** | **ai** | **ua** | **ün** | **★a系完美对应★**（a → ai → ua）+ ün互补 |
| 3 | an | ian | uai | uan | **an系列** + uai + uan互补分布 |
| 4 | **ang** | **iang** | **uang** | **eng** | **★ang系列完美垂直★** |
| 5 | o | 空 | ou | uo | **o系**（o + u = ou/uo）|
| 6 | e | ei | ui | üe | **e系** + ui + ü系 |
| 7 | en | ie | iu | ing | en + i系(ie/iu) + ing |
| 8 | ao | iao | iong | ong | **ao系** + iong/ong后鼻音 |
| 9 | 空 | in | 空 | ␣ | in + 空格 |
| 10 | 空 | ia | ⇧ | ⌫ | i系 + 控制键 |

**核心正交关系**：
- **ai = a(1,2) + i(2,1)**：行1列2的a + 行2列1的i = 行2列2的ai ✓
- **ua = u(3,1) + a(1,2)**：行3列1的u + 行1列2的a = 行3列2的ua ✓
- **ei = e(1,6) + i(2,1)**：行1列6的e + 行2列1的i = 行2列6的ei ✓
- **ou = o(1,5) + u(3,1)**：行1列5的o + 行3列1的u = 行3列5的ou ✓
- **ian = an(1,3) + i(2,1)**：行1列3的an + 行2列1的i = 行2列3的ian ✓
- **iang = ang(1,4) + i(2,1)**：行1列4的ang + 行2列1的i = 行2列4的iang ✓

**关键垂直对应（核心设计）**：
- **列2**：a → ai → ua → ün（★完美a系垂直★）✓✓✓
- **列4**：ang → iang → uang → eng（后鼻音家族完美垂直）✓✓✓
- **列1**：er → i → u → ü（基础元音锚点）✓
- **列3**：an → ian → uai → uan（前鼻音系列 + uai/uan）✓
