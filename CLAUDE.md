...
- 行4列2：**ün** - 互补分布（j/q/x/y 层显示 "ün"，其他层显示 "un"）
- 行4列7：**üan** - 互补分布（j/q/x/y 层显示 "üan"，其他层显示 "uan"）
- 行4列8：**␣** - 空格键
- 行4列9：**⌫** - 删除键
...
**ün 和 un 的互补**：
- **ün**：可配声母 j/q/x/y（jün, qün, xün, yün - 军、群、寻、云）
- **un**：可配声母 b/p/m/f/d/t/n/l/g/k/h/zh/ch/sh/r/z/c/s/w（如 lun论, kun困）
- **共用位置**：行4列2
- **显示规则**：
  - j/q/x/y 层：显示 "ün"
  - 其他声母层：显示 "un"
- **输入规则**：
  - j/q/x/y 层：输入 jvn/qvn/xvn/yvn（RIME 中 v=ü）
  - 其他声母层：输入 Xun（如 lun, kun）

**üan 和 uan 的互补**：
- **üan**：可配声母 j/q/x/y（jüan, qüan, xüan, yüan - 捐、全、选、元）
- **uan**：可配声母 b/p/m/f/d/t/n/l/g/k/h/zh/ch/sh/r/z/c/s/w（如 luan乱, guan关）
- **共用位置**：行4列7
- **显示规则**：
  - j/q/x/y 层：显示 "üan"
  - 其他声母层：显示 "uan"
- **输入规则**：
  - j/q/x/y 层：输入 jvan/qvan/xvan/yvan
  - 其他声母层：输入 Xuan（如 luan, guan）
...
| 声母层 | 行4列2位置 | 行4列7位置 | 说明 |
|-------|----------|----------|------|
| shengyun_finals | ün | üan | 通用层 |
| shengyun_finals_j | ün | üan | j 层如实显示 |
| shengyun_finals_l | un | uan | l 层互补显示 |
| shengyun_finals_zh | un | uan | 翘舌音层互补显示 |
